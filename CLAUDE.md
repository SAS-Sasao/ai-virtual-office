...
- UI/UX モック（フロントモック v3）: `docs/design/ui/`（画面・配色・状態表現の参照仕様。正本は claude.ai/design プロジェクト、詳細は同ディレクトリ README）。**描画方式は参照しないこと** — モックは一枚絵背景だが実装は手続き描画が正（ADR-002）
- AWS 構成図（draw.io）: `docs/design/aws-architecture.drawio`
- AWS 構成図（CloudFormation 相当 YAML）: `docs/design/aws-architecture.cfn.yaml`
- ループエンジニアリング設計（検証 hooks / SKILL / E2E 自動化）: `docs/design/loop-engineering-design.md`（`.claude` 構成は M0 着手前セット導入済み。以降の導入順序は同書 §6 を参照）
- 設計決定ログ（ADR）: `docs/design/decision-log.md`（ADR-001: コアアプリの Docker 不採用と限定利用 2 用途 / ADR-002: オフィス描画は手続き描画が正・背景画像は任意レイヤー）
...
```bash
pnpm dev     # apps/web の dev サーバ起動（http://localhost:3001。3000 は本環境で占有中のため）
pnpm relay   # Relay 起動（http://localhost:4100。hooks の唯一の宛先。protocol/relay を build してから起動）
pnpm build   # 全パッケージビルド（pnpm -r run build）
