...
pnpm test    # vitest（pnpm -r run test。現在は web の game/lib 層）
```

M0 実装済み: hooks → `/api/ingest`（正規化・機微破棄）→ SSE `/api/stream` → Canvas 最小描画。スキーマは暫定的に `apps/web/game/protocol.ts` にあり、**M1 で `packages/protocol` へ抽出予定**。
