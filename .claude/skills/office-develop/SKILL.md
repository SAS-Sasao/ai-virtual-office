...
4. [2026-07-20 / M1-2a] **Phase 3 の実機検証で `next dev -p 3001` を環境変数付きで起動したつもりが、ポートが既存プロセスに占有されて起動に失敗し、環境変数なしの古いサーバが応答していた。** 隔離したはずの検証が本番 DB に書き込み、気づくまで数回やり直した

**運用注記**: 上記から導かれる最小限の手順のみを本文に足した（Phase 3 の事前ステージ確認 / 設計時の実行環境チェック / 検証と常駐プロセスの資源分離 / 実機検証の専用ポート）。以後も失敗が起きたら日付付きで追記してから本文を足す。全面リライトは禁止。
...
**実機検証は dogfooding が使うポート（web 3001 / Relay 4100）を避け、専用ポート（3005 / 4105 等）を使う**（ギャップ記録 4）。起動直後にログを `grep EADDRINUSE` で確認し、意図したプロセスが応答していることを確かめてから検証する。
