...
**フックはパッケージ単位で発火し、触った特定ファイルとは無関係にパッケージ全体の tsc 結果を見る**（M1-4a rev.2 で確認、2026-07-25）。protocol の `RoomSchema` に `door` を必須化した直後、`import-org.ts` の Room 構築 2 箇所がまだ未対応で型エラーが残っている状態で `fixtures/masters.ts`（無関係な追加のみ）や `import-org.test.ts`（`*.test.ts` でフック対象外のはずの内容）を編集しても、**そのつど同じ `import-org.ts` のエラーを理由にフックがブロックメッセージを返した**。ただし観測した限り、**ブロックされても Write/Edit 自体は実際にはディスクへ反映されていた**（PostToolUse は事後通知であり、ツール呼び出し自体をロールバックしない）。したがって、複数ファイルにまたがる破壊的スキーマ変更（今回の door 必須化のような）を進める際は、ブロックメッセージが出ても慌てず「最終的に全部揃った時点で改めて `tsc --noEmit` が通ること」を確認すればよい。ただし正式な完了報告前には必ずクリーンな `tsc --noEmit` exit 0 を自分で再実行して確認すること（ブロックメッセージの有無に頼らない）。
