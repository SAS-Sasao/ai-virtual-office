...
| ポート 4100 | 空き | ✅ | — |
| Playwright ブラウザ | 本体導入済み（chromium-1228 / headless_shell-1228 / ffmpeg-1011。2026-07-19 §3 前倒し実行） | ⚠️ | 起動には WSL 側ライブラリ `libnss3` `libnspr4` `libasound2` の apt 導入（sudo・人間対応）が必要。導入後にヘッドレス起動を再検証する |
...
| sqlite3 CLI | 未導入 | ❌（任意） | M1 以降・任意（3-6） |
