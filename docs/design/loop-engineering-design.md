...
4. ヒットしたら exit 2:
   "packages/game/ は React 非依存が規約（NFR-7）。該当 import を削除し、
...
```
1. stop_hook_active ガード（同上）
2. apps/web/ または packages/game/ に diff がなければ exit 0
...
|---|---|---|---|---|
| game-engine-dev | maker | sonnet | Read, Write, Edit, Glob, Grep, Bash | packages/game 専任（既計画） |
...
- **nightly**: full E2E + visual + flake 集計 → 失敗時は AI 修復ループ B を claude-code-action で起動
...
| MS | 導入するもの |
|---|---|
| **M0 着手前** | guard-game-react / typecheck-touched hooks、office-verify 骨格（build + 起動確認のみ）、office-qa agent、settings.json permissions |
| **M0→M1** | Debug State API（状態機械と同時実装）、fixture 注入エンドポイント、gate-protocol-consumers、protocol-change スキル、@smoke 3 本 + gate-e2e-smoke、e2e-authoring スキル、gate-test-weakening、CI PR ゲート |
