...
| 思想的ベース | 【Loop Engineering (Addy Osmani)】(https://addyosmani.com/blog/loop-engineering/) / 【スキル自動最適化とループエンジニアリング (LayerX)】(https://zenn.dev/layerx/articles/9f25ec86a31730) / 【AIに「レビューして」はもう古い？「敵対的検証」のすすめ (Loglass)】(https://zenn.dev/loglass/articles/6aa18c80496ec6) |
| ステータス | レビュー待ち |
...
### 4.2 敵対的検証プロトコル（office-qa のレビュー手順）

office-qa のレビューは「指摘リストを返すレビュー」ではなく**敵対的検証**として実施する（【AIに「レビューして」はもう古い？「敵対的検証」のすすめ (Loglass)】(https://zenn.dev/loglass/articles/6aa18c80496ec6) の 4 要件 — 独立性・反証の役割づけ・接地・判断できる出力 — を採用）:

| 要件 | 本設計での実装 |
|---|---|
| 独立性（fresh context） | office-qa は Task で新規起動され、maker の作業文脈を引き継がない。maker の自己申告（PR 本文・コミットメッセージ・「テスト通りました」）は**検証対象の主張であって根拠ではない** |
| 反証の役割づけ | 「この成果物は間違っている」を作業仮説とし、主張・前提・結論の壊し方を探す。作者の意図・努力は考慮せず、良い点の列挙もしない（採点根拠に必要な事実の記載を除く） |
| 接地（grounding） | 事実の主張は必ず一次情報で裏取りする — コードは該当行の Read、実行結果は自分で実行した exit code / 出力、設計準拠は docs/design/ の該当節。§4.1 の s1 実測義務はこの原則の機械面への適用 |
| 判断できる出力 | findings の各要素を **claim（指摘の一言）/ severity（high・medium・low）/ evidence（一次情報の出典）/ confidence（high・medium・low）** の 4 項目で構造化する（§4.1 の findings 配列の要素スキーマ）。verdict JSON の直前に**反証ログ**（試みた反証と結果・壊せなかった点）を必須で記載する |

**運用ルール（記事の既知の失敗モードへの対策）**:

- **偽陽性抑制**: 確信のない指摘は findings に入れない。指摘は自分で反証を試みて生き残ったものだけを報告する（偽陽性の多い checker はノイズの多い損失関数となり、自己修正ループを収束させない — §4.1 と同旨）
- **逆方向失敗（手抜き合格）ガード**: 反証ログの無い findings 空を pass にしない。攻撃の記録がない「合格」は検証不十分として扱う（甘い pass は偽陽性より危険）
- **マルチレンズ検証（任意・高リスク時）**: protocol 変更・リリース前など、やり直しコストが高い成果物では、呼び出し側が観点別（正確性 / セキュリティ / 決定論性など）に複数の office-qa を並列起動し判定を突き合わせてよい。判定ルール（§4.1）は各レンズにそのまま適用する
...
| 敵対的検証 | 「成果物は間違っている」を作業仮説に反証を試みる検証手法（§4.2）。fresh context・一次情報への接地・構造化 findings・反証ログを要件とする |
| 反証ログ | office-qa が verdict JSON の直前に記載する「試みた反証と結果・壊せなかった点」の記録（§4.2）。これが無い findings 空は pass にしない |
