...
### 5.4 (B) ai-virtual-office リポジトリ自身の `.claude` 構成
...
ai-virtual-office/
└── .claude/
    ├── settings.json          ← 開発用 permissions（pnpm/vitest 許可等）+ 自アプリの hooks を dogfooding 導入
    ├── agents/
    │   ├── game-engine-dev.md      ← Canvas/状態機械/経路探索 担当（game/ 配下専任）
    │   ├── pipeline-dev.md         ← Relay/protocol/ingest 担当
    │   ├── ui-dev.md               ← React シェル/画面 担当
    │   └── org-adapter-dev.md      ← cc-sier-adapter/インポータ 担当
    └── skills/
        ├── office-verify/          ← 起動 → ダミーイベント投入 → 描画確認の E2E 手順
        └── office-release/         ← ビルド・タグ・（将来）Vercel デプロイ手順
```

- **dogfooding**: 本リポジトリでの Claude Code 開発作業自体を自アプリで観測する（開発中の最良のテストデータになる）
- CLAUDE.md には「`game/` React 非依存」「protocol 変更時は relay/web 両テスト」「素材ライセンス表記維持」等の制約を記載
- Subagent の分割は上記 4 領域（モジュール境界と一致させ、並列開発を可能にする）
